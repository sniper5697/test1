...
- 다음 단계는 deploy target secret과 restart strategy를 채우고 첫 수동 배포를 검증하는 것이다.
...
## Last Completed Task
- GitHub `main` branch protection을 실제 저장소에 적용했고, `staging`/`production` environment와 수동 self-hosted deploy skeleton workflow를 추가했다.
...
- 지금부터는 deploy target secret 입력과 first deploy smoke 확인 순서로 넘기는 것이 자연스럽다.
